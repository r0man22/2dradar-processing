...
while(cap.isOpened()):
    ...
    for contour in contours:
        ...
        # Artı işaretini tespit etmek için köşe sayısına bakma
        if len(approx) >= 12:  # Artı işareti için genellikle çok sayıda kenar vardır
            (x, y, w, h) = cv2.boundingRect(contour)
            
            # Aspect ratio kontrolü
            aspect_ratio = w / h
            if aspect_ratio > 0.8 and aspect_ratio < 1.2:  # Simetrik, kareye yakın şekiller
                # Konturun şekli artı işareti gibi görünüyorsa
